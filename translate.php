...
# Logic: ignore empty/whitespace-only, maintain 1:1 positions
$norm = []; $pos = [];
foreach ($texts as $i=>$t) {
  $s = (string)$t;
  $trim = trim(strip_tags($s));
  if ($trim === '') { $norm[]=''; $pos[]=$i; }
  else { $norm[]=$s; $pos[]=$i; }
}
if (!$norm) { echo json_encode(['translations'=>[]]); exit; }
...
function http_json($url, $method='POST', $headers=[], $body=null) {
  $ch = curl_init($url);
  curl_setopt_array($ch, [CURLOPT_CUSTOMREQUEST=>$method, CURLOPT_RETURNTRANSFER=>true, CURLOPT_HTTPHEADER=>$headers, CURLOPT_POSTFIELDS=>$body, CURLOPT_TIMEOUT=>60]);
...
function do_translate($translator, $texts, $source, $target, $cfg){
  switch ($translator) {
    ...
    case 'mock': default: return array_map(function($t) use($target){ return $t===''?'':("[MOCK-$target] ".$t); }, $texts);
  }
}
...
if ($total > $maxCharsReq) {
  ...
  $out = do_translate($translator, $norm, $source, $target, $cfg);
}

# Final: enforce same count, fill empties for skipped rows
$out = array_values($out);
for ($i=0; $i<count($out); $i++) { if (!isset($out[$i]) || !is_string($out[$i])) $out[$i]=''; }
if (count($out) !== count($norm)) {
  # Align lengths to prevent UI misplacement
  if (count($out) < count($norm)) { $out = array_pad($out, count($norm), ''); }
  else { $out = array_slice($out, 0, count($norm)); }
}
...
function translate_libre(array $texts, string $source, string $target, array $cfg): array {
  ...
  $outputs = [];
  foreach ($texts as $t) {
    $trim = trim(strip_tags((string)$t));
    if ($trim === '') { $outputs[]=''; continue; }
    $parts = split_text_for_limit((string)$t, $maxChars);
    $translatedParts = [];
    foreach ($parts as $part) {
      $translatedParts[] = libre_call_with_retry($endpoint, $apiKey, $part, $source, $target, $delayMs);
    }
...
function libre_call_with_retry($endpoint,$apiKey,$q,$source,$target,$delayMs){
  ...
  while (true) {
    ...
    if ($resp!==false && $http<429 && $http<500) { $j=json_decode($resp,true); if ($delayMs>0) usleep($delayMs*1000); return (string)($j['translatedText'] ?? ''); }
...
function split_text_for_limit(string $text, int $maxChars): array {
  ...
  $chunks=[]; $current=''; $len = mb_strlen($text,'UTF-8');
  for($i=0;$i<$len;$i++){
...
function translate_mymemory(array $texts,string $source,string $target,array $cfg): array {
  $out=[]; foreach ($texts as $q) {
    $url = 'https://api.mymemory.translated.net/get?q='.rawurlencode($q).'&langpair='.rawurlencode(($source?:'auto').'|'.$target);
    [$http,$resp,$err] = http_json($url,'GET',[]); if ($resp===false || $http>=400) { $out[]=$q; continue; }
    $data = json_decode($resp,true); $out[] = (string)($data['responseData']['translatedText'] ?? $q);
  } return $out;
}
