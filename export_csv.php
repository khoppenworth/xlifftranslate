...
$units = $parsed['units'] ?? []; $rows = [];
if ($ids) { $map=[]; foreach ($units as $u) $map[$u['id']]=$u; foreach ($ids as $id) if (isset($map[$id])) { $u=$map[$id]; $rows[] = [$id, strip_tags($u['source']), (string)($targets[$id] ?? $u['target'] ?? '')]; } }
else { foreach ($units as $u) { $id=$u['id']; $rows[] = [$id, strip_tags($u['source']), (string)($targets[$id] ?? $u['target'] ?? '')]; } }
